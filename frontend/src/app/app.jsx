...
type AppState = 'patient-info' | 'chat' | 'analyzing' | 'pre-registration' | 'result';
type Language = 'en' | 'bis';

export default function App() {
  const [state, setState] = useState<AppState>('patient-info');
  ...
  const [registrationData, setRegistrationData] = useState<RegistrationData | undefined>();

  // Triage data
  const [triageLevel, setTriageLevel] = useState<0 | 1 | 2 | 3>(2);
  ...
  const handlePatientInfoSubmit = (name: string, age: number, isPWD: boolean) => {
    setPatientName(name);
    setPatientAge(age);
  ...
  const handleSymptomSubmit = async (symptomText: string) => {
    ...
    let level: 0 | 1 | 2 | 3 = 2; // Default to non-urgent
